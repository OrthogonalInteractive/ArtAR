...
- 配置後も画面中央が別の認識済み壁を向くと「ここに飾る」が再表示され、その壁へ移動できます。認識した壁を消さずに切り替えられます。「壁の再検出」は壁と配置を消して検出をやり直します。
...
- 認識済みの壁を向きごとに色分け。濃い面・格子で観測範囲、薄い面・破線で推定延長した配置範囲を常時表示。作品の四隅と「つかんで移動」の案内を表示し、ドラッグ中は壁色を強調。手動の実寸補正・配置範囲指定は撤去。
- 安定した壁の一部が見つかれば、同じ平面を上下左右へ延長して配置可能にします。未検出の方向は観測範囲から左右各2m・上下各1.5mまで。天井・床の候補や隣の壁が確認できた方向は交差線で区切ります。
...
通常のAR画面でも、確定した壁を向きごとに色分けします。直交する壁は水色と橙色などで区別し、同じ壁の色はカメラ移動・作品切替でも維持します。未配置時とドラッグ中は濃く、配置後は薄く表示します。同じ壁の中では、観測した点の凸包を濃い面・格子、その外へ延長した範囲を薄い面・破線で区別します。推定範囲は観測済み・実在する壁の範囲を保証するものではありません。
...
- 向きごとに色分けした面：複数回一致した壁。白枠は作品を配置している壁です。
...
深度・ヒットテスト・空間特徴点の点群は、3点RANSAC・重力方向・残差・面内の分布で選別します。床を壁として扱わないよう、元の3点法線が水平に近い候補だけを使います。平面を再推定した後で面内点も再判定します。同一面20点以上、かつ処理対象点全体の12%以上を必要とし、抽出後の残り点から偶然できた面も同じ点数条件で判定します。面を3×3に分け、2点以上ある区画が6区画以上、縦横それぞれ全3列に分布すること、および凸包の面積が外接矩形の35%以上あることを確認します。離れた縁だけを結んだ面や細い斜めの点列を広い壁として採用しません。

ネイティブ面も含め650msごとに判定し、最初の観測から位置4cm・向き3度以内で3回連続一致した壁を採用します。直前の観測との一致だけで、少しずつずれる面が確定することを防ぎます。一致が途切れる・追跡を失うと未確定の一致回数を数え直し、未確定の候補だけを2.5秒で失効させます。iOSの空間特徴点については、同じ位置で同じ点群を再処理するだけでは確定せず、端末が6cm以上移動しても面の位置・向きが一致することを必要とします。Depthとネイティブ面にこの移動条件はありません。

確定済みの壁は観測が途切れても保持します。確定時の位置・向きを維持し、上記の位置・向きの安定条件を満たす実際の観測だけで範囲の凸包を追加します。配置後も観測範囲は追加できますが、平面の位置・向きは固定します。採用条件を厳しくした分、模様が少ない壁や狭くしか映していない壁は認識まで時間がかかる場合があります。家具・カーテンと壁の意味的な識別は行いません。

配置には観測ポリゴンとは別の有限な延長範囲を使います。境界が不明な方向は左右各2m・上下各1.5mまで延長します。隣接する壁・水平な天井や床の候補が確認できた場合、観測範囲から最大4m以内の交差線まで延長し、そこで区切ります。隣の壁は両方の観測範囲が同じ室内側にあり、高さが近く、両面の交差位置が有限な到達範囲にある場合だけ利用します。水平面は壁面から40cm以内まで近づいた観測ポリゴンを必要とします。斜め天井や曲面、ドア・窓・家具の開口／障害物は自動識別しません。

Androidのネイティブ水平面と、Android／iOSの点群から天井・床の候補を検出します。点群は40点以上・水平の広がり各60cm以上・面積0.5㎡以上・面内分布・高さの残差で選別し、高さ3cm以内で3回連続一致してから境界に使います。iOSの特徴点では6cm以上の視点移動も必要です。カメラから45cm以上上を天井候補、90cm以上下を床候補に限定し、ネイティブの意味ラベルが家具等を示す場合は除外します。意味ラベルが得られない面を家具と完全に区別するものではありません。水平面は配置対象の壁数に含めず、観測が途切れても保持します。Depth／ネイティブ面がないAndroidのヒットテスト点群は垂直面だけなので、天井・床の境界は取得できません。

推定延長を検出根拠や観測点に戻すことはありません。延長範囲内で新たに独立検出した点群が、確定済みの壁と位置4cm・向き3度以内で一致する場合は、同じ壁IDへ統合します。後から境界が判明した場合は額縁全体が収まる位置へ補正し、作品自体が入らない場合は配置を解除します。作品の実寸や壁の位置・向きは変えません。デバッグには確認済みの天井・床候補の数を表示します。

同一壁の照合には、向きと奥行きに加えてポリゴンの重なりを利用します。同じ向きで深度差10cm未満の重なる／隣接する観測は同じ面に統合します。15度以内・深度差35cm以内で、小さい方のポリゴンの55%以上が重なる候補は重複の疑いとして抑制し、確定済みの壁を動かしたり範囲を広げたりしません。これらは誤認識を抑える判定値で、寸法精度の保証ではありません。直交面・離れた平行面・重ならない面は別の壁として扱います。ごく近接した実物の平行面も抑制する場合があり、家具と壁の意味的な識別は未実装です。

WebXRのネイティブ面と点群からの推定が重なった場合はネイティブ面を優先します。未確定の推定を別のネイティブ面へ置き換える場合は一致回数を数え直します。確定済みの面は後から得た推定で手前へ移動させません。通常表示・配置判定・デバッグの候補表示に同じ照合規則を使います。

一時的に端末の追跡が失われた場合は、壁・作品の表示と操作を止めてデータを保持し、追跡復帰後に再表示します。「壁の再検出」、AR終了、WebXRの座標系リセットでは古い壁と配置を破棄します。保持はそのARセッション内だけで、次回起動やページ再読込には引き継ぎません。

誤った平面へ画面中央を向けると「平面を削除」を使えます。強調されている平面だけを削除し、そこに作品があれば、残った確定済みの平面のうち額縁全体が収まる位置が元の作品位置に最も近い面へ移します。作品のサイズを保ち、移動先の面に角度を合わせます。収まる面がなければ配置を解除します。作品のない平面を削除しても、別の面にある作品を自動で移し替えません。削除後は残った壁の推定延長と境界制約も再計算します。

左上の「平面を非表示」で、認識した面の色・格子・輪郭・推定延長、中央の目印、作品の操作枠とラベルを隠せます。認識データと作品の配置は保持し、非表示中も作品のドラッグ・切り替えができます。追跡復帰や追加検出でも非表示を維持し、「平面を表示」で戻します。見えない面を削除しないよう、非表示中は平面の削除・配置ボタンも隠します。デバッグの点群・平面・パネルも一時的に隠し、デバッグを明示的にオンにすると平面表示も戻ります。「壁の再検出」やARの再開始では平面表示をオンに戻します。

削除した面と位置4cm・向き3度以内で一致し、削除時の有限な延長範囲に重なる／隣接する観測は、そのARセッションで再採用しません。通常の重複抑制より狭い条件で除外するため、角度や奥行きが修正された別の検出を受け入れられます。削除済みのネイティブ面が端末から返り続けても、修正されたDepth推定を妨げません。診断の平面表示からも除外します。「壁の再検出」、AR終了、WebXR座標リセットでこの除外も消えます。追跡喪失中・ドラッグ中は削除できません。

- 濃い壁ガイドは観測された範囲、薄いガイドと破線は推定延長です。境界が見つかるまでは実物の端を越える場合があります。面検出APIから得たポリゴンも含め、物理的な壁の端や飾れる範囲を確定するものではありません。
- 白一色、反射、暗所、ブレ、特徴点の少ない壁は認識できないことがあります。模様・壁の角・貼った目印を映して移動してください。
- 窓・ドア・家具の意味理解、未認識の隣接壁、実物によるオクルージョンは未実装です。色のついた面が物理的に飾れる場所かは目視で確認してください。
