...
- 認識済みの壁を向きごとに色分けした面・輪郭・格子で常時表示。作品の四隅と「つかんで移動」の案内を表示し、ドラッグ中は壁色を強調。手動の実寸補正・配置範囲指定は撤去。
...
通常のAR画面でも、確定した壁を向きごとに色分けします。直交する壁は水色と橙色などで区別し、同じ壁の色はカメラ移動・作品切替でも維持します。未配置時とドラッグ中は濃く、配置後は薄く表示します。表示範囲は観測した面を元にした凸包です。
...
確定済みの壁は観測が途切れても保持します。確定時の位置・向きを維持し、未配置の壁は上記の位置・向きの安定条件を満たす観測だけで範囲の凸包を追加します。配置後は範囲も固定します。採用条件を厳しくした分、模様が少ない壁や狭くしか映していない壁は認識まで時間がかかる場合があります。家具・カーテンと壁の意味的な識別は行いません。
