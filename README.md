...
深度・ヒットテスト・空間特徴点の点群は、3点RANSAC・重力方向・残差・面の広がりで選別します。床を壁として扱わないよう、元の3点法線が水平に近い候補だけを使います。ネイティブ面も含め、650msごとに判定し、3回以上一致した壁を採用、選択後は面を固定します。WebXRの座標系がリセットされた場合は、古い配置を解除して再認識します。
...
- WebXRはメートル単位、8th WallはAbsolute Scaleを利用します。いずれも計測精度を保証しません。メジャーで測った距離による補正を用意しています。ピンチによる自由拡大は実寸を崩すため用意していません。
